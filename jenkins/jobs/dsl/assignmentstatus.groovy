...
def assignmentstatus_FolderName = pertask_FolderName + "/Manage_Assignment_Status"
...
def status = freeStyleJob(assignmentstatus_FolderName + "/Manage_Assignment_Status")
...
retrieveConfig.with{
  ...
  authenticationToken('TWFuYWdlQXNzaWdubWVudFN0YXR1cw==')
  scm{
    git{
      remote{
        url(hcmCoreConfig)
  ...
  publishers{
    downstreamParameterized{
      trigger(assignmentstatus_FolderName + "/Manage_Assignment_Status"){
        condition("SUCCESS")
		  parameters{
...
status.with{
    ...
    steps {
		...
		shell('''#!/bin/bash
java -jar /var/jenkins_home/jobs/Oracle/jobs/HCM/jobs/HCM-Core_per_Task/jobs/Manage_Assignment_Status/jobs/Manage_Assignment_Status/workspace/target/HCM-0.0.1-SNAPSHOT.jar -r "Manage Assignment Status" -w $WORKSPACE -e /var/jenkins_home/jobs/Oracle/jobs/HCM/jobs/HCM-Core_per_Task/jobs/Manage_Assignment_Status/jobs/Retrieve_Configuration/workspace
cd ..
mkdir screenshots 
cd screenshots       
