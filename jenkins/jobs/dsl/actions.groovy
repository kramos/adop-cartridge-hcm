...
def actions_FolderName = pertask_FolderName + "/Manage_Actions"
...
// Jobs
def retrieveConfig = freeStyleJob(actions_FolderName + "/Retrieve_Configuration")
def actions = freeStyleJob(actions_FolderName + "/Manage_Actions")

// Pipeline
def actions_pipeline = buildPipelineView(actions_FolderName + "/Manage_Actions")

actions_pipeline.with{
    title('Manage Actions')
    displayedBuilds(5)
    selectedJob(actions_FolderName + "/Retrieve_Configuration")
...
retrieveConfig.with{
  ...
  authenticationToken('TWFuYWdlQWN0aW9ucw==')
  scm{
    git{
      remote{
        url(hcmCoreConfig)
  ...
  publishers{
    downstreamParameterized{
      trigger(actions_FolderName + "/Manage_Actions"){
        condition("SUCCESS")
		  parameters{
